...
### Work in progress
def main():
    osrs.move.move_and_click(800, 800, 9, 9)
    keyboard.send('f4')
    osrs.clock.random_sleep(1, 1.1)
    loc = osrs.util.rough_img_compare('..\\screens\\burning_amulet.png', 0.9, (0, 0, 1920, 1080))
    if not loc:
        return
    osrs.move.move_and_click(loc[0], loc[1], 5, 5, 'right')
    osrs.clock.random_sleep(1, 1.1)
    loc = osrs.util.rough_img_compare('..\\screens\\lava_maze.png', .99, (0, 0, 1920, 1080))
    if not loc:
        return
    osrs.move.move_and_click(loc[0], loc[1], 2, 2)
    osrs.clock.random_sleep(1.3, 1.5)
    keyboard.send('1')
