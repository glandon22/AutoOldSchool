...
def click_towards_sack():
    while True:
        data = osrs.server.query_game_data(queries.I3)
        if 'tiles' in data and '375056620' in data['tiles'] and data['tiles']['375056620']['y'] < 1010:
            osrs.move.move_and_click(data['tiles']['375056620']['x'], data['tiles']['375056620']['y'], 7, 7)
            break

def click_closest_ore_vein():
    while True:
        data = osrs.server.query_game_data(queries.UPPER_ORE_VEINS)
...
def look_north_and_zoom_out():
    osrs.server.query_game_data(queries.SET_YAW_NORTH)
...
def click_upper_ladder():
    while True:
        data = osrs.server.query_game_data(queries.UPPER_LADDER)
        if 'gameObjects' in data and '19045' in data['gameObjects']:
            osrs.move.move_and_click(data['gameObjects']['19045']['x'], data['gameObjects']['19045']['y'], 5, 5)
        osrs.clock.random_sleep(0.1, 0.2)
        data = osrs.server.query_game_data(queries.CLICKED_OBJECT)
...
def click_lower_ladder():
    while True:
        data = osrs.server.query_game_data(queries.LOWER_LADDER)
        if 'gameObjects' in data and '19044' in data['gameObjects']:
            osrs.move.move_and_click(data['gameObjects']['19044']['x'], data['gameObjects']['19044']['y'] - 15, 5, 5)
        osrs.clock.random_sleep(0.1, 0.2)
        data = osrs.server.query_game_data(queries.CLICKED_OBJECT)
...
def bank_and_return():
    ...
    osrs.move.wait_until_stationary()
    osrs.clock.random_sleep(2.0, 2.1)
...
def main():
    ...
    while True:
        start_time = osrs.game.break_manager(start_time, 53, 59, 432, 673, 'pass_71', look_north_and_zoom_out)
