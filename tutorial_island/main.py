...
def main():
    general_utils.bezierMovement(1200,1230,30,240)
    general_utils.randomSleep(0.1, 0.2)
    pyautogui.click()
    ...
    general_utils.bezierMovement(look_up_name[0] + 3, look_up_name[0] + 36, look_up_name[1] + 2, look_up_name[1] + 7)
    general_utils.randomSleep(0.2, 0.3)
    pyautogui.click()
    general_utils.randomSleep(0.2, 0.3)
    general_utils.bezierMovement(1250,1275, 200,300)
    general_utils.randomSleep(4.5, 5.6)
    ...
    general_utils.bezierMovement(set_name[0] + 3, set_name[0] + 36, set_name[1] + 2, set_name[1] + 7)
    general_utils.randomSleep(0.2, 0.3)
    pyautogui.click()
    general_utils.randomSleep(4.5,6.1)
...
def main1():
    ...
    for loc in arrow_locs:
        general_utils.bezierMovement(loc[0], loc[1], loc[2], loc[3])
        for i in range(0, random.randint(1, 5)):
            general_utils.randomSleep(0.4,0.5)
            pyautogui.click()
    general_utils.bezierMovement(1102, 1186, 761, 776)
...
def main3():
    while True:
        ...
        general_utils.randomSleep(2.1,2.5)
        guide = general_utils.rough_img_compare('..\\screens\\tutorial_island\\gielinor_guide.png', .8, ())
        if guide:
            break
    for i in range(1,6 ):
        keyboard.send('space')
        general_utils.randomSleep(0.9,1.1)
    keyboard.send('1')
    for i in range(3):
        general_utils.randomSleep(0.4,0.6)
        keyboard.send('space')
    general_utils.randomSleep(0.4, 0.6)
    general_utils.bezierMovement(2449, 2466, 1357, 1377)
    general_utils.randomSleep(0.2,0.3)
    pyautogui.click()
    while True:
        general_utils.find_moving_target_no_verify(np.array(pyscreenshot.grab()))
    ...
    general_utils.bezierMovement(2446, 2466, 214, 220)
    general_utils.randomSleep(0.1, 0.2)
    pyautogui.click()
    ...
    while True:
        ...
        if survival:
            break
    keyboard.send('space')
    general_utils.bezierMovement(2162, 2178, 1356, 1376)
    general_utils.randomSleep(0.2,0.3)
    pyautogui.click()
    ...
    while True:
        ...
        if shrimp:
            break
    general_utils.change_npc_highlights('survival expert')
    general_utils.bezierMovement(2076, 2092, 1355, 1374)
    general_utils.randomSleep(0.2, 0.3)
    pyautogui.click()
    ...
    while True:
        ...
        if logs:
            log_loc = logs
            break
        general_utils.randomSleep(1.0,1.1)

    tind = general_utils.rough_img_compare('..\\screens\\tutorial_island\\tinderbox.png', .8, ())
    if not tind:
        print('no tinderbox')
    general_utils.bezierMovement(tind[0] + 1,tind[0] + 5,tind[1] + 2,tind[1] + 7)
    general_utils.randomSleep(0.2,0.3)
    pyautogui.click()
    ...
    if not logs_bag:
        print('no logs_bag')
    general_utils.bezierMovement(logs_bag[0] + 1,logs_bag[0] + 5,logs_bag[1] + 2,logs_bag[1] + 7)
    general_utils.randomSleep(0.2,0.3)
    pyautogui.click()
    ...
    if not shrimp:
        print('no logs_bag')
    general_utils.bezierMovement(shrimp[0] + 1, shrimp[0] + 5, shrimp[1] + 2, shrimp[1] + 7)
    general_utils.randomSleep(0.4, 0.5)
    pyautogui.click()
    general_utils.bezierMovement(1279,1281,719,721)
    general_utils.randomSleep(0.1,0.2)
    pyautogui.click()
general_utils.find_fixed_object(np.array(pyscreenshot.grab()), 0, 0)