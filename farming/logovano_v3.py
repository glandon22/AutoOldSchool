...
logo_seed_id = osrs.item_ids.LOGAVANO_SEED
LOGOVANO_SEED = '13425'
EMPTY_PATCH = '27383'
        ...
        while True:
            ...
            if desired_state == WATERED_STAGE_1:
                if qh.get_objects(osrs.queryHelper.ObjectTypes.GAME.value, UNWATERED_SEED_STAGE_1) and \
                    ...
                    logo_seed = qh.get_inventory(logo_seed_id)
                    if not logo_seed:
...
def get_seeds():
    ...
    if qh.get_widgets(osrs.widget_ids.chat_input_request_widget):
        ...
        return True
    elif qh.get_chat_options('Logavano', fuzzy=True):
        osrs.keeb.write(str(qh.get_chat_options('Logavano', fuzzy=True)))
...
def main(endless_loop=True):
    ...
    while True:
        ...
        osrs.move.interact_with_object_v3(
            ...
            custom_exit_function_arg=osrs.item_ids.LOGAVANO_FRUIT
        )
