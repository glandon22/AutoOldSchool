...
def platform_1(lc):
    ...
    if 'z' in loc and loc['z'] == 3 and 3097 <= loc['x'] <= 3102 and 3277 <= loc['y'] <= 3281 and click_allowed(lc, 3, 2):
        osrs.agil.handle_marks(3097, 3102, 3277, 3281, 3, port)
        obstacle = osrs.server.get_ground_object('3098,3277,3', '11405', port)
        if bool(obstacle):
...
def platform_2(lc):
    ...
    if 'z' in loc and loc['z'] == 3 and 3087 <= loc['x'] <= 3093 and 3272 <= loc['y'] <= 3278 and click_allowed(lc, 3, 3):
        osrs.agil.handle_marks(3087, 3093, 3272, 3278, 3, port)
        obstacle = osrs.server.get_ground_object('3092,3276,3', '11406', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
...
def platform_3(lc):
    ...
    if 'z' in loc and loc['z'] == 3 and 3089 <= loc['x'] <= 3094 and 3265 <= loc['y'] <= 3268 and click_allowed(lc, 3, 4):
        osrs.agil.handle_marks(3089, 3094, 3265, 3268, 3, port)
        obstacle = osrs.server.get_game_object('3089,3264,3', '11430', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
...
def platform_4(lc):
    ...
    if 'z' in loc and loc['z'] == 3 and 3083 <= loc['x'] <= 3088 and 3256 <= loc['y'] <= 3261 and click_allowed(lc, 3, 5):
        osrs.agil.handle_marks(3083, 3088, 3256, 3261, 3, port)
        obstacle = osrs.server.get_game_object('3088,3256,3', '11630', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
...
def platform_5(lc):
    ...
    if 'z' in loc and loc['z'] == 3 and 3087 <= loc['x'] <= 3094 and 3252 <= loc['y'] <= 3255 and click_allowed(lc, 3, 6):
        osrs.agil.handle_marks(3087, 3094, 3252, 3255, 3, port)
        obstacle = osrs.server.get_game_object('3095,3255,3', '11631', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
...
def platform_6(lc):
    ...
    if 'z' in loc and loc['z'] == 3 and 3096 <= loc['x'] <= 3101 and 3256 <= loc['y'] <= 3261 and click_allowed(lc, 3, 7):
        osrs.agil.handle_marks(3096, 3101, 3256, 3261, 3, port)
        obstacle = osrs.server.get_game_object('3102,3261,3', '11632', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
