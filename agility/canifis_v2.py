...
def start_course(lc):
    ...
    if 'z' in loc and loc['z'] == 0 and click_allowed(lc, 10, 1):
        ...
        if 'tiles' in data and '350834890' in data['tiles']:
            osrs.move.move_and_click(data['tiles']['350834890']['x'],
...
def platform_1(lc):
    ...
    if 'z' in loc and loc['z'] == 2 and 3504 <= loc['x'] <= 3511 and 3491 <= loc['y'] <= 3497 and click_allowed(lc, 3, 2):
        osrs.agil.handle_marks(3504, 3511, 3491, 3497, 2, port)
        obstacle = osrs.server.get_game_object('3506,3498,2', '14844', port)
        if bool(obstacle):
...
def platform_2(lc):
    ...
    if 'z' in loc and loc['z'] == 2 and 3496 <= loc['x'] <= 3504 and 3503 <= loc['y'] <= 3507 and click_allowed(lc, 3, 3):
        osrs.agil.handle_marks(3496, 3504, 3503, 3507, 2, port)
        obstacle = osrs.server.get_game_object('3496,3504,2', '14845', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
...
def platform_3(lc):
    ...
    if 'z' in loc and loc['z'] == 2 and \
        ...
        osrs.clock.sleep_one_tick()
        osrs.agil.handle_marks(3485, 3493, 3498, 3505, 2, port)
        obstacle = osrs.server.get_game_object('3485,3499,2', '14848', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
...
def platform_4(lc):
    ...
    if 'z' in loc and loc['z'] == 3 and\
        ...
        osrs.agil.handle_marks(3474, 3480, 3491, 3500, 3, port)
        obstacle = osrs.server.get_game_object('3478,3491,3', '14846', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
...
def platform_5(lc):
    ...
    if 'z' in loc and loc['z'] == 2 and \
        ...
        osrs.agil.handle_marks(3477, 3484, 3481, 3487, 2, port)
        obstacle = osrs.server.get_game_object('3480,3483,2', '14894', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
...
def platform_6(lc):
    ...
    if 'z' in loc and loc['z'] == 3 and \
        ...
        osrs.agil.handle_marks(3488, 3503, 3468, 3478, 3, port)
        obstacle = osrs.server.get_game_object('3504,3476,3', '14847', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
...
def platform_7(lc):
    ...
    if 'z' in loc and loc['z'] == 2 and \
        ...
        osrs.agil.handle_marks(3508, 3516, 3474, 3483, 2, port)
        obstacle = osrs.server.get_game_object('3510,3483,2', '14897', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
