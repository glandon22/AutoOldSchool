...
def start_course(lc):
    ...
    if 'z' in loc and loc['z'] == 0 and click_allowed(lc, 3, 1):
        ...
        if 'decorativeObjects' in data and '14927' in data['decorativeObjects']:
            osrs.move.move_and_click(data['decorativeObjects']['14927'][0]['x'],
...
def platform_1(lc):
    ...
    if 'z' in loc and loc['z'] == 3 and \
        ...
        osrs.agil.handle_marks(2721, 2730, 3490, 3497, 3, port)
        obstacle = osrs.server.get_game_object('2720,3494,3', '14928', port)
        if bool(obstacle):
...
def platform_2(lc):
    ...
    if 'z' in loc and loc['z'] == 2 and \
        ...
        osrs.agil.handle_marks(2704, 2714, 3487, 3498, 2, port)
        obstacle = osrs.server.get_ground_object('2710,3489,2', '14932', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
...
def platform_3(lc):
    ...
    if 'z' in loc and loc['z'] == 2 and \
        ...
        osrs.agil.handle_marks(2709, 2716, 3476, 3482, 2, port)
        obstacle = osrs.server.get_game_object('2711,3476,2', '14929', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
...
def platform_4(lc):
    ...
    if 'z' in loc and loc['z'] == 3 and \
        ...
        osrs.clock.sleep_one_tick()
        osrs.clock.sleep_one_tick()
        osrs.agil.handle_marks(2699, 2716, 3469, 3476, 3, port)
        obstacle = osrs.server.get_game_object('2701,3469,3', '14930', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
...
def platform_5(lc):
    ...
    if 'z' in loc and loc['z'] == 2 and \
        ...
        osrs.agil.handle_marks(2691, 2703, 3459, 3466, 2, port)
        obstacle = osrs.server.get_game_object('2703,3463,2', '14931', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            osrs.clock.random_sleep(1, 1.2)
            return {
