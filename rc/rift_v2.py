...
def take_uncharged_cells(status_obj):
    ...
    uncharged_cell = osrs.inv.is_item_in_inventory_v2(inv, 26882)
    if in_main_arena() \
            and (not game_active() or is_pregame()) \
...
def enter_arena(status_obj):
    if outside_main_arena() and click_allowed(status_obj, '43700', 5):
...
def main():
    clicks = {}
    while True:
        clicks = enter_arena(clicks)
        clicks = take_uncharged_cells(clicks)
