...
def game_active():
    elem = osrs.server.get_widget('746,23')
    return bool(elem)
...
def take_uncharged_cells(status_obj):
    ...
    if in_main_arena() and not game_active() and not uncharged_cell:
        table = osrs.server.get_game_object('3618,9488,0', '43732', port)
        if table:
            osrs.move.right_click_menu_select(table, None, port, 'Uncharged cells', 'Take-10')
            return {**status_obj, '43732': datetime.datetime.now()}
        else:
            osrs.move.run_towards_square_v2({'x': 3618, 'y': 9488, 'z': 0}, port)
    return status_obj
...
def run_to_large_remains(status_obj):
    ...
    inv = osrs.inv.get_inv(port)
    uncharged_cell = osrs.inv.is_item_in_inventory_v2(inv, 26882)
    if (not game_active() or is_pregame()) and osrs.server.get_target_obj(port) != 43724 and in_main_arena() and uncharged_cell:
        obstacle = osrs.server.get_ground_object('3634,9503,0', '43724', port)
        if bool(obstacle):
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
            return {**status_obj, '43724': datetime.datetime.now()}
        else:
            osrs.move.run_towards_square_v2({'x': 3626, 'y': 9500, 'z': 0}, port)
    return status_obj
...
def mine_large_remains(status_obj):
    ...
    if 'x' in loc and loc['x'] >= 3637 and \
            ...
            and not osrs.server.is_mining(port) and game_active(): # and osrs.server.get_target_obj(port) != 43719
        rock = osrs.server.get_game_object('3640,9498,0', '43719', port)
        if rock:
...
def leave_large_remains_area(status_obj):
    ...
    if 'x' in loc and loc['x'] >= 3637 and \
            ...
            and beginning and 'spriteID' in beginning and beginning['spriteID'] != 4369 and osrs.server.get_target_obj(port) != 43726:
        obstacle = osrs.server.get_ground_object('3636,9503,0', '43726', port)
        if obstacle:
            osrs.move.move_and_click(obstacle['x'], obstacle['y'], 3, 3)
...
def frag_port_available(status_obj):
    ...
    portal_text = osrs.server.get_widget('746,28', port)
    loc = osrs.server.get_world_location(port)
    inv = osrs.inv.get_inv(port)
    if portal_text and loc and 'x' in loc and 3597 <= loc['x'] <= 3633 and len(inv) < 28 and osrs.server.get_target_obj(port) != 43729:
        text_parts = portal_text['text'].split(' - ')
        if len(text_parts) >= 2 and text_parts[1] != '0:00':
            print('b', text_parts[0])
            osrs.move.run_towards_square_v2(frag_portal_dict[text_parts[0]], port)
            orb = osrs.server.get_surrounding_game_objects(10, ['43729'], port)
...
def mine_huge_remains(status_obj):
    ...
    loc = osrs.server.get_world_location(port)
    inv = osrs.inv.get_inv(port)
    if loc and 'x' in loc and loc['x'] <= 3594 and len(inv) < 28 and not osrs.server.is_mining(port) and osrs.server.get_target_obj(port) != 43720 and game_active():
        remains = osrs.server.get_surrounding_game_objects(8, ['43720'], port)
...
def leave_huge_remains(status_obj):
    ...
    loc = osrs.server.get_world_location(port)
    inv = osrs.inv.get_inv(port)
    if loc and 'x' in loc and loc['x'] <= 3594 and (len(inv) == 28 or not game_active()) and osrs.server.get_target_obj(port) != 38044:
        remains = osrs.server.get_game_object('3593,9503,0', '38044', port)
        if remains:
...
def make_essence():
    ...
    if in_main_arena() and guardian_frags and not guardian_ess and not elem_guardian_stone:
        work_table = osrs.server.get_surrounding_game_objects(10, ['43754'], port)
        if not bool(work_table):
            osrs.move.run_towards_square_v2({'x': 3615, 'y': 9492, 'z': 0}, port)
        work_table = osrs.server.get_surrounding_game_objects(10, ['43754'], port)
        if work_table and '43754' in work_table:
            osrs.move.move_and_click(work_table['43754']['x'], work_table['43754']['y'], 3, 3)
            if int(osrs.server.get_target_obj(port)) == 43754:
                start_time = datetime.datetime.now()
                while True:
                    curr_inv = osrs.inv.get_inv(port)
                    guardian_frags = osrs.inv.is_item_in_inventory_v2(curr_inv, 26878)
...
def make_essencev2(status_obj):
    ...
    guardian_ess = osrs.inv.is_item_in_inventory_v2(inv, 26879)
    elem_guardian_stone = osrs.inv.is_item_in_inventory_v2(inv, 26881)
    if in_main_arena() and guardian_frags and not guardian_ess and not elem_guardian_stone and osrs.server.get_player_animation(port) != 9365:
        if osrs.inv.are_items_in_inventory_v2(inv, [554, 555, 556, 557]): #  and osrs.server.get_target_obj(port) != 43696
            print('have runes')
            rune_deposit = osrs.server.get_game_object('3609,9487,0', '43696', port)
            if rune_deposit:
                print('found depo')
                osrs.move.move_and_click(rune_deposit['x'], rune_deposit['y'], 3, 3)
                return {**status_obj, '43696': datetime.datetime.now()}
            else:
                osrs.move.run_towards_square_v2({'x': 3609, 'y': 9487, 'z': 0}, port)
        else:
            workbench = osrs.server.get_game_object('3612,9487,0', '43754', port)
            if workbench:
                osrs.move.move_and_click(workbench['x'], workbench['y'], 3, 3)
                return {**status_obj, '43754': datetime.datetime.now()}
            else:
                osrs.move.run_towards_square_v2({'x': 3612, 'y': 9487, 'z': 0}, port)
    return status_obj
...
def enter_active_rift(status_obj):
    ...
    guardian_ess = osrs.inv.is_item_in_inventory_v2(inv, 26879)
    elem_guardian_stone = osrs.inv.is_item_in_inventory_v2(inv, 26881)
    if in_main_arena() and guardian_ess and osrs.server.get_player_animation(port) != 9365 and not elem_guardian_stone:
        ...
        if active_elem_rift and 'spriteID' in active_elem_rift and \
                ...
                str(osrs.server.get_target_obj(port)) != sprite_to_rift_obj_dict[str(active_elem_rift['spriteID'])]['id']:
            ...
            if rift:
                ...
                osrs.move.run_towards_square_v2(rift_to_enter['tile_obj'], port)
    return status_obj
...
def make_waters(status_obj):
    inv = osrs.inv.get_inv(port)
    guardian_ess = osrs.inv.is_item_in_inventory_v2(inv, 26879)
    if in_water_altar() and guardian_ess and osrs.server.get_target_obj(port) != 34762:
        altar = osrs.server.get_game_object('2716,4836,0', '34762', port)
        if altar:
            osrs.move.move_and_click(altar['x'], altar['y'], 3, 3)
            return {**status_obj, '34762': datetime.datetime.now()}
        else:
            osrs.move.run_towards_square_v2({'x': 2716, 'y': 4836, 'z': 0}, port)
    return status_obj
...
def make_earths(status_obj):
    inv = osrs.inv.get_inv(port)
    guardian_ess = osrs.inv.is_item_in_inventory_v2(inv, 26879)
    if in_earth_altar() and guardian_ess and osrs.server.get_target_obj(port) != 34763:
        altar = osrs.server.get_game_object('2658,4841,0', '34763', port)
        if altar:
            osrs.move.move_and_click(altar['x'], altar['y'], 3, 3)
            return {**status_obj, '34763': datetime.datetime.now()}
        else:
            osrs.move.run_towards_square_v2({'x': 2658, 'y': 4841, 'z': 0}, port)
    return status_obj
...
def make_airs(status_obj):
    inv = osrs.inv.get_inv(port)
    guardian_ess = osrs.inv.is_item_in_inventory_v2(inv, 26879)
    if in_air_altar() and guardian_ess and osrs.server.get_target_obj(port) != 34760:
        altar = osrs.server.get_game_object('2844,4834,0', '34760', port)
        if altar:
            osrs.move.move_and_click(altar['x'], altar['y'], 3, 3)
            return {**status_obj, '34760': datetime.datetime.now()}
        else:
            osrs.move.run_towards_square_v2({'x': 2844, 'y': 4834, 'z': 0}, port)
    return status_obj
...
def make_fires(status_obj):
    inv = osrs.inv.get_inv(port)
    guardian_ess = osrs.inv.is_item_in_inventory_v2(inv, 26879)
    if in_fire_altar() and guardian_ess and osrs.server.get_target_obj(port) != 34764:
        altar = osrs.server.get_game_object('2585,4838,0', '34764', port)
        if altar:
            osrs.move.move_and_click(altar['x'], altar['y'], 3, 3)
            return {**status_obj, '34764': datetime.datetime.now()}
        else:
            osrs.move.run_towards_square_v2({'x': 2585, 'y': 4838, 'z': 0}, port)
    return status_obj
...
def leave_fire_altar(status_obj):
    inv = osrs.inv.get_inv(port)
    guardian_ess = osrs.inv.is_item_in_inventory_v2(inv, 26879)
    if in_fire_altar() and not guardian_ess and osrs.server.get_target_obj(port) != 34752:
        exit = osrs.server.get_game_object('2574,4850,0', '34752', port)
        if exit:
            osrs.move.move_and_click(exit['x'], exit['y'], 3, 3)
            return {**status_obj, '34752': datetime.datetime.now()}
        else:
            osrs.move.run_towards_square_v2({'x': 2574, 'y': 4850, 'z': 0}, port)
    return status_obj
...
def leave_water_altar(status_obj):
    inv = osrs.inv.get_inv(port)
    guardian_ess = osrs.inv.is_item_in_inventory_v2(inv, 26879)
    if in_water_altar() and not guardian_ess and osrs.server.get_target_obj(port) != 34750:
        exit = osrs.server.get_game_object('2727,4832,0', '34750', port)
        if exit:
            osrs.move.move_and_click(exit['x'], exit['y'], 3, 3)
            return {**status_obj, '34750': datetime.datetime.now()}
        else:
            osrs.move.run_towards_square_v2({'x': 2727, 'y': 4832, 'z': 0}, port)
    return status_obj
...
def leave_air_altar(status_obj):
    inv = osrs.inv.get_inv(port)
    guardian_ess = osrs.inv.is_item_in_inventory_v2(inv, 26879)
    if in_air_altar() and not guardian_ess and osrs.server.get_target_obj(port) != 34748:
        exit = osrs.server.get_game_object('2841,4828,0', '34748', port)
        if exit:
            osrs.move.move_and_click(exit['x'], exit['y'], 3, 3)
            return {**status_obj, '34748': datetime.datetime.now()}
        else:
            osrs.move.run_towards_square_v2({'x': 2841, 'y': 4828, 'z': 0}, port)
    return status_obj
...
def leave_earth_altar(status_obj):
    inv = osrs.inv.get_inv(port)
    guardian_ess = osrs.inv.is_item_in_inventory_v2(inv, 26879)
    if in_earth_altar() and not guardian_ess  and osrs.server.get_target_obj(port) != 34751:
        exit = osrs.server.get_game_object('2655,4829,0', '34751', port)
        if exit:
            osrs.move.move_and_click(exit['x'], exit['y'], 3, 3)
            return {**status_obj, '34751': datetime.datetime.now()}
        else:
            osrs.move.run_towards_square_v2({'x': 2655, 'y': 4829, 'z': 0}, port)
    return status_obj
...
def charge_guardian(status_obj):
    inv = osrs.inv.get_inv(port)
    elem_guardian_stone = osrs.inv.is_item_in_inventory_v2(inv, 26881)
    if in_main_arena() and elem_guardian_stone and osrs.server.get_target_npc(port) != 11403:
        guardian = osrs.server.get_npcs_by_id('11403', port)
        if len(guardian) > 0:
            osrs.move.move_and_click(guardian[0]['x'], guardian[0]['y'], 3, 3)
            return {**status_obj, '11403': datetime.datetime.now()}
        else:
            osrs.move.run_towards_square_v2({'x': 3615, 'y': 9503, 'z': 0}, port)
    return status_obj
