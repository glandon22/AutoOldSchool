...
import move
import npc
'''invoking_file = inspect.stack()[1][1]
file_w_extension = invoking_file[invoking_file.rindex('/')+1:]
trimmed_file_name = file_w_extension[:file_w_extension.rindex('.')]'''
keyboard = Controller()
...
def check_and_dismiss_random(random_data):
    ...
    if len(random_data) == 0:
        return 'no randoms'
    move.move_and_click(math.floor(random_data[0]['x']), math.floor(random_data[0]['y']), 7, 7, 'right')
    random_sleep(1, 1.1)
    dismiss = rough_img_compare('..\\screens\\dismiss.png', .9, (0, 0, 1920, 1080))
    if dismiss:
        move.move_and_click(int(dismiss[0]), int(dismiss[1]), 4, 4)
        return 'success'
    else:
...
def click_banker(port='56799'):
    ...
    data = query_game_data(q, port)
    if len(data["npcs"]) != 0:
        closest = npc.find_closest_npc(data['npcs'])
