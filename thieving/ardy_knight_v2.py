...
def hop_worlds():
    for world in world_list:
        osrs.keeb.keyboard.press(osrs.keeb.Key.enter)
        osrs.keeb.keyboard.release(osrs.keeb.Key.enter)
        osrs.keeb.keyboard.type('::hop {}'.format(world))
        osrs.keeb.keyboard.press(osrs.keeb.Key.enter)
        osrs.keeb.keyboard.release(osrs.keeb.Key.enter)
        ...
        if knight:
            ...
            # verify knight is in bank
            if 2655 >= closest['x_coord'] >= 2649 and 3287 >= closest['y_coord'] >= 3280:
                return


#not working, randomly hops worlds - think i fixed it
def steal_knight():
    ...
    for i in range(clicks):
        ...
        knight = osrs.server.get_npcs_by_id('3297,11936', '56799')
        if knight: # also need to check the knight is inside bank
            closest = osrs.util.find_closest_npc(knight)
            # verify knight is in bank
            print(closest)
            if 2655 >= closest['x_coord'] >= 2649 and 3287 >= closest['y_coord'] >= 3280:
